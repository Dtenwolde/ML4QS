...
TYPES = ["On table", "Cycling", "Walking"]
for type in TYPES:
    DATASET_PATH = Path('./datasets/' + type)
    RESULT_PATH = Path('./intermediate_datafiles/')
    RESULT_FNAME = 'chapter2_result_own.csv'

    # Set a granularity (the discrete step size of our time series data). We'll use a course-grained granularity of one
    # instance per minute, and a fine-grained one with four instances per second.
    GRANULARITIES = [15000, 250]
    ...
    for milliseconds_per_instance in GRANULARITIES:
        ...
        #
        # # Plot all data
        DataViz.plot_dataset(dataset, ['acc_', 'gyr_', 'light_phone_lux', 'mag_'],
                                       # 'hr_watch_rate', 'light_phone_lux', 'mag_', 'press_phone_', 'label'],
                                      ['like', 'like', 'like', 'like'],
                                 # , 'like', 'like', 'like', 'like', 'like','like'],
                                      ['line', 'line', 'line', 'line'], type, milliseconds_per_instance)
